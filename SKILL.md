...
service (domain) ──depends on──>  nada. Cero imports de Spring, MongoDB, HTTP.
```

Si una clase en `service/` (el paquete raíz, no `service/impl` ni
`service/ports`) importa algo de `org.springframework.*`,
`org.springframework.data.mongodb.*`, o cualquier librería HTTP, **está en
el paquete equivocado**. El dominio no sabe que existe una base de datos, un
...
| `controller/impl/` | Controladores REST. Reciben DTOs, llaman a un `XxxUseCase`, devuelven DTOs. Nunca tocan documentos Mongo ni lógica de negocio directamente. | `XxxController` |
| `repository/adapter/` | Implementaciones concretas de los puertos de salida: MongoDB, HTTP hacia Mercado Pago, etc. | `XxxRepositoryAdapter`, `XxxGatewayAdapter` |
| `repository/mongo/` | Interfaces `MongoRepository` de Spring Data — solo acceso crudo a la colección, sin lógica. | `XxxMongoRepository extends MongoRepository<...>` |
| `document/` | Documentos `@Document` de Spring Data MongoDB — persistencia pura, sin comportamiento de negocio. Nunca salen de `repository/adapter/` ni `mapper/`. | `XxxDocument` |
| `mapper/` | Traducción explícita entre capas: dominio ↔ documento Mongo, dominio ↔ DTO. Clases estáticas, sin estado. | `XxxPersistenceMapper` (dominio↔documento), `XxxRestMapper` (dominio↔DTO) |
...
2. **Puerto de entrada** (`service/ports/XxxUseCase.java`): la interfaz
   ...
   dominio (nunca recibe ni devuelve DTOs ni documentos Mongo).
...
- **Filtrar un `XxxDocument` (MongoDB) fuera de `repository/adapter/` o
  `mapper/`.** Si un `Service` o `Controller` importa algo de
  `document/`, es un error — deben trabajar solo con el objeto de dominio.
...
- **Optimistic locking:** cualquier entidad que pueda ser modificada por
  ...
  el webhook y por el job de expiración) debe tener
  `@org.springframework.data.annotation.Version` en su `XxxDocument`, y
  el `Service` debe capturar `org.springframework.dao.OptimisticLockingFailureException`
  como un no-op silencioso, no como un error a propagar — MongoDB lanza
  directamente esa clase (no una subclase específica de un ORM), a
  diferencia de JPA/Hibernate.
